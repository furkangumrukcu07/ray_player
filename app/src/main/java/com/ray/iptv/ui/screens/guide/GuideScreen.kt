        ...
        Row(Modifier.fillMaxSize(), horizontalArrangement = Arrangement.spacedBy(14.dp)) {
            GlassPanel(strong = true, radius = 18.dp, modifier = Modifier.weight(1f).fillMaxSize()) {
                LazyColumn(Modifier.padding(10.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                    items(channels.take(200), key = { it.id }) { ch ->
                        GlassButton((if (selected?.id == ch.id) "●  " else "") + ch.name) { selected = ch }
                    }
                }
            }
                ...
                LazyColumn(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    items(programmes, key = { it.id }) { p ->
                        ...
                        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                            ...
                            if (live) GlassButton(copy.watch) { selected?.let(onPlay) }
                            else if (past && (selected?.hasArchive == true)) GlassButton(copy.catchup) { selected?.let { onCatchup(it, p) } }
                        }
                    }
                }
            }
